"""
3D Convolutional Variational Autoencoder for Spin System Analysis

This module implements the complete 3D VAE architecture that combines the 3D encoder
and decoder components with proper loss functions for unsupervised learning
of order parameters in 3D spin systems.
"""
...
class ConvolutionalVAE3D(nn.Module):
    """
    Complete 3D Convolutional Variational Autoencoder.
    
    Combines 3D encoder and decoder networks with ELBO loss function for
    unsupervised learning of latent representations of 3D spin configurations.
    Supports β-VAE formulation for controlling the balance between
    reconstruction and regularization. Maintains identical hyperparameters
    (β=1.0) from 2D implementation.
    """
    ...
    def __init__(
        ...
        """
        Initialize the 3D Convolutional VAE.
        
        Args:
            input_shape: Shape of input tensor (channels, depth, height, width)
        ...
        """
        super(ConvolutionalVAE3D, self).__init__()
        
        self.input_shape = input_shape
        self.latent_dim = latent_dim
        self.beta = beta
        
        # Initialize 3D encoder and decoder
        self.encoder = ConvolutionalEncoder3D(
        ...
        self.decoder = ConvolutionalDecoder3D(
            latent_dim=latent_dim,
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Forward pass through the complete 3D VAE.
    ...
    def sample(self, num_samples: int, device: Optional[torch.device] = None) -> torch.Tensor:
        ...
        Returns:
            Generated samples of shape (num_samples, channels, depth, height, width)
        """
        if device is None:
